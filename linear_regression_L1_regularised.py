...
class LinearRegression:
    ...
    def _get_MSE(self, y_hat, y, rf):
        ...
        """
        return np.mean((y_hat - y)**2) + rf*sum(abs(self.w))
    ...
    def fit(self, X, y, X_val, y_val, lr = 0.001, epochs=1000,
            acceptable_error=0.001, print_loss=False, rf=0.7):
        """
        ...
        for epoch in range(epochs):
            ...
            for X_batch, y_batch in minibatches:
                ...
                self.w -= lr * grad_w + rf*np.sign(self.w)
                self.b -= lr * grad_b
        ...
        if print_loss:
            ...
            plt.plot(mean_loss, label="Minibatch loss: L1")
            plt.plot(mean_validation_loss, label="Validation loss: L1")
            plt.legend()
            plt.show()
